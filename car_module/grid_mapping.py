...
class MappingBot(YouBot):
    ...
    def read_ref(self):
        self.value = self.sim.getObject("/youBot_ref")
        x, y = self.sim.getObjectPosition(self.value)[:2]
        print(x,y)
...
class Grid:
    ...
    def mapping(self, loc, scan):
        ...
        ]  # grid[row, col] = grid[y_range, x_range]
        ...
        gx = (
            np.arange(i_min, i_max + 1) * 0.1 + 0.05 - 5
        )  # numpy.arange([start, ] stop, [step, ] dtype=None)
        gx = np.repeat(
            gx.reshape(1, -1), sub_grid.shape[0], axis=0
        )  # gx 배열을 1*N 형태로 변환.
        ...
        gd = (
            dx**2 + dy**2
        ) ** 0.5  # 라이다 위치로부터 sub grid 각 셀 까지의 거리 추정.

        # theta diff
        # 각 지점과 로봇의 스캔 세서 사이의 각도 차이를 구하고, 로봇이 바라보는 방향 theta 와 비교해 각도 차이 dtheta 를 계산.
        gtheta = np.arccos(dx / gd) * (
            (dy > 0) * 2 - 1
        )  # gtheta : global coord.origin 에 대한 광선의 각도
        dtheta = (
            gtheta - theta
        )  # dtheta : 로봇의 yaw 를 고려한 광선의 상대 각도. 로봇 이동 방향 기준으로 계산된 광선의 각도.
        # 각도 범위 조정. (-pi ~ pi)
        ...
        for i in range(13):
            ...
            if (
                res == 0
            ):  # 장애물이 없는 경우 -> 해당 방향에서 특정 거리 내에 있는 셀들을 자유 공간으로 간주. free area
                area = (
                    (
                        gd <= 2.25
                    )  # sub gird 각 셀과 로봇 센서 사이의 거리가 2.25 미터 이하인지 확인.
                    * (
                        -self.boundary + self.delta * i <= dtheta
                    )  # i 번째 스캔 데이터가 커버하는 각도 범위 내에 있는 그리드 셀만을 업데이트 대상으로.
                    * (dtheta <= -self.boundary + self.delta * (i + 1))
                )
                sub_grid[
                    area
                ] -= 0.5  # 위 조건을 만족하는 area 에서 그리드 셀의 값을 0.5 만큼 감소 -> 자유공간 가능성.
            else:  # 장애물 감지.
                dist = min(2.25, dist)  # dist 재정의.
                detect_area = (
                    (
                        np.abs(gd - dist) < self.min_dist
                    )  # 그리드 상의 각 지점과 로봇 센서 사이의 거리가 탐지된 거리와 매우 근접한지 여부 확인. 기준 값 : 0.07071
                    * (-self.boundary + self.delta * i <= dtheta)
                    * (dtheta <= -self.boundary + self.delta * (i + 1))
                )
                sub_grid[
                    detect_area
                ] += 0.5  # 그리드 값을 0.5만큼 증가 -> 장애물 있을 가능성 큼.
                free_area = (
                ...
                sub_grid[
                    free_area
                ] -= 0.5  # 장애물 있을 가능성 존재 -> 자유 공간 셀 가중치 감소.
        np.clip(
            self.grid, -5, 5, out=self.grid
        )  # 마지막으로 그리드의 모든 셀 값이 -5에서 5 사이로 제한.

    def save(self):
        with open(
            "/home/oh/my_coppeliasim/with-robot-3rd/car_module/mapping.npy", "wb"
        ) as f:
            np.save(f, self.grid)
